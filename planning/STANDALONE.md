...
| ~~`Intl` under Hermes still unverified~~ **Settled on device 2026-08-27, and it was three questions, not one:** `Intl.Collator` ✅ and `Intl.DateTimeFormat` ✅ both work; **`Intl.RelativeTimeFormat` does not exist** and crashed the app on opening any anime. Shimmed in `mobile/src/polyfills.ts`, checked against Node's ICU over 443 pairs. | Carried over from `planning/PLAN.md`. **The same class of bug bit three times now — twice caught by reading source, once only by the crash:** React Native has no `AbortSignal.timeout`, which *all three* shared clients (`anilist/client`, `mal/client`, `nyaa/rss`) call. Fixed with a shim in `mobile/src/polyfills.ts` rather than a fork, because the point is to share the domain layer. Treat "it imports nothing" as necessary but not sufficient for portability — the runtime has to be checked too. |
| ~~AniList's implicit grant returns the token in a URL *fragment*~~ | **Retired 2026-08-27.** Android 16 hands the fragment back through `openAuthSessionAsync` intact: sign-in completed and 854 titles imported. The redirect *also* reaches expo-router, which needed routes at both redirect paths — see `mobile/src/app/auth/`. |
