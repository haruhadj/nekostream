...
| Android builds are local (Gradle) rather than EAS cloud, and APKs are re-signed after the build instead of configuring signing in `build.gradle` | The operator asked for local builds after two cloud builds queued ~50 minutes each — Phase 5 needs many install-test-adjust cycles against Doze, which that cadence makes miserable. Toolchain: scoop `android-clt` + temurin17 (AGP does not accept the JDK 25 on PATH). Signing sits in `mobile/scripts/local-release.sh` because `android/` is generated by `expo prebuild` and gitignored — anything written into it is lost on the next regeneration, whereas re-signing the finished APK with `apksigner` survives, and keeps every local build on one signature so each installs over the last. **Switching off the EAS-signed APK costs exactly one uninstall**, which takes the device database and both tracker logins with it. | Aug 28, 2026 |
